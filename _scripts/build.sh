...
usage() {
    ...
    echo "  $0 --release --target-dir ./target2       # 指定输出目录"
...
# stealth feature 需要 cmake（BoringSSL 编译依赖），检测后自动下载
STEALTH_MODE=0
if echo "$CARGO_EXTRA_ARGS" | grep -q 'stealth'; then
    STEALTH_MODE=1
fi
...
if [[ "$BD_VENDOR_ONLY" == "1" ]]; then
    ...
    if [[ "$BD_SKIP_VENDOR" -eq 0 ]]; then
        # 如果 V8 源码已存在于 third_party，说明 vendor 已完成，跳过重复下载
        # 只有当 V8 缺失时才需要运行 vendor（第一个 vendor 块也会处理此情况）
        if [[ -d "$V8_SRC_DIR" ]]; then
...
if [[ "$STEALTH_MODE" -eq 1 ]] && [[ "$BD_SKIP_CMAKE" -eq 0 ]]; then
    download_cmake || {
        echo -e "${RED}========================================${NC}"
...
elif [[ "$STEALTH_MODE" -eq 1 ]] && [[ "$BD_SKIP_CMAKE" -eq 1 ]]; then
...
if [[ "$STEALTH_MODE" -eq 1 ]] && [[ -x "$CMAKE_DIR/bin/cmake" ]]; then
...
if [[ "$STEALTH_MODE" -eq 1 ]] && [[ "$CCACHE_ENABLED" == "1" ]]; then
...
# ===================== 编译 =====================
# 确保 cargo 配置文件存在（V8 源已存在时 create_cargo_configs 不会被调用）
...
# 组装最终编译命令
BUILD_CMD="V8_FROM_SOURCE=1 PRINT_GN_ARGS=true cargo build ${CARGO_EXTRA_ARGS} ${BUILD_MODE} --target ${TARGET_LINK} --target-dir ${TARGET_DIR}"
...
if [[ "$BUILD_V8_EXAMPLES" == "1" ]]; then
    ...
    V8_EXAMPLES_CMD="V8_FROM_SOURCE=1 cargo build -p v8 --examples ${CARGO_EXTRA_ARGS} ${BUILD_MODE} --target ${TARGET_LINK} --target-dir ${TARGET_DIR}"
    echo -e "${BLUE}$V8_EXAMPLES_CMD${NC}"
    V8_FROM_SOURCE=1 cargo build -p v8 --examples ${CARGO_EXTRA_ARGS} ${BUILD_MODE} --target ${TARGET_LINK} --target-dir ${TARGET_DIR} 2>&1 | tee ${TARGET_DIR}/examples_build.log
