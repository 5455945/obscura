...
_check_rust_latest() {
    ...
    local LOCAL_VERSION
    LOCAL_VERSION=$(basename "$RUST_DIR" | grep -oP 'rust-\K[0-9]+\.[0-9]+\.[0-9]+')
    ...
    LATEST_VERSION=$(echo "$HOST_URL" | grep -oP 'rustc?-\K[0-9]+\.[0-9]+\.[0-9]+')
...
download_rust_toolchain() {
    ...
    DOWNLOAD_DATE=$(echo "$HOST_URL" | grep -oP 'dist/\K[0-9]{4}-[0-9]{2}-[0-9]{2}')
    RUST_VERSION=$(echo "$HOST_URL" | grep -oP 'rustc?-\K[0-9]+\.[0-9]+\.[0-9]+')
...
download_libclang() {
    ...
    LIBCLANG_VERSION=$(grep "^RELEASE_VERSION" "$V8_CLANG_UPDATE" | grep -oP "'\K[0-9]+")
    ...
    if [[ -f "$CLANG_DIR/lib/libclang-${LIBCLANG_VERSION}.so.${LIBCLANG_VERSION}" ]]; then
        ...
        local LIBCLANG_URL
        LIBCLANG_URL=$(curl -s "$APT_INDEX_URL" | grep -oP "href=\"\Klibclang1-${LIBCLANG_VERSION}[^\"]*amd64\.deb" | tail -1)
